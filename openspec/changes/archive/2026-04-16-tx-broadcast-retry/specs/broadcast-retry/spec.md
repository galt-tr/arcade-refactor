...
The propagation service SHALL classify broadcast failures as either retryable or permanent based on the Teranode endpoint error response. Errors containing "missing inputs" or "txn-mempool-conflict" SHALL be classified as retryable. All other broadcast errors SHALL be classified as permanent.
...
On service startup, the propagation service SHALL query the store for transactions with `PENDING_RETRY` status and re-enqueue them into the retry buffer.
...
- **THEN** the system SHALL load those transactions into the retry buffer for continued retry processing
